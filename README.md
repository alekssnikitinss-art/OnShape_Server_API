...
Status Code: 400_". Also it is able to check API working statuss. 
  ...
  (10.12.2025) Created new functions, that will be needing updates bcs functionallity is limited. Created simple documenation, for how to use the app, how it is made. Research was made on OnShapes Rest api, using documenation, tried to test document id and see  what variables can be used, how diffrent comand lines work on it, how they are made. Test was succsesfull but did give much values, since there no in numbers values, that can be evalueted, to understand how suffieciet is the app. In condut, the app might not work. There might be some overlaping features or functions, soo the both repo and directiory will need some cleaning. Maybe updating files even, to see if that makes anything better. Also added new infomamtion to diffrent documents and reread some, to understand the programm better.  
  (11.12.2025) Trying wiht a new account, a profesional one, nothing really changed in the wayvalues are gotten, New functions for experimmeted porupuses also did work.  
  (12.12.2025) Created a freatureScript code, and began testing it, to see how relible are values and if it is possible to get the nessecary values are shown in config variables. 
